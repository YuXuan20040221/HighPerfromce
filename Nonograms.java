...
public class Nonograms {
    ...
    public static void main(String[] args) {
        try {
            int[][] G = new int[X][X];
            String[][] ans = new String[2][X];
            File file = new File("Nonograms.txt");
    ...
    static int[] Paint1(int i, int j, LinkedList<Integer> D, int[] L) {
        ...
        if (i == dj) {
            ...
            result = Paint0(i - dj, j - 1, d, result);
        }
    ...
    static Boolean Fix1(int i, int j, LinkedList<Integer> D, int[] L) {
        ...
        if (i == 0 && j == 0) {
            ...
            // 確保在 i - dj 之前的格子沒有其他填充條件
            for (int k = i - dj - 1; k >= 0; k--) {
                if (L[k] == 1 && d.isEmpty()) {
                    // System.out.println("Fix1:F5");
                    return false;
                } else if (L[k] == 1) {
                    k -= d.removeLast();
                    j--;
